...
inline bool Syslog::_sendLog(uint16_t pri, const char *message) {
  ...
  snprintf_P(syslog_preamble, sizeof(syslog_preamble), PSTR("%s ESP-%s "), this->_deviceHostname, this->_appName);

  this->_client->print(syslog_preamble);
  this->_client->print(message);
  this->_client->endPacket();

  Serial.printf("%s ESP-%s: %s \n", this->_deviceHostname, this->_appName, message);
  yield();
  return true;
}
